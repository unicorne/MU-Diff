...
def train_mudiff(rank, gpu, args):
    ...
    val_dice_loss = np.zeros([args.num_epoch, len(data_loader_val)])
    val_dice_loss1 = np.zeros([args.num_epoch, len(data_loader_val)])
    val_dice_loss2 = np.zeros([args.num_epoch, len(data_loader_val)])
    val_dice_loss3 = np.zeros([args.num_epoch, len(data_loader_val)])
    val_smooth_loss = np.zeros([args.num_epoch, len(data_loader_val)])
    val_smooth_loss1 = np.zeros([args.num_epoch, len(data_loader_val)])
    val_smooth_loss2 = np.zeros([args.num_epoch, len(data_loader_val)])
    val_smooth_loss3 = np.zeros([args.num_epoch, len(data_loader_val)])
    ...
    broadcast_params(reg_net.parameters())
    ...
    for epoch in range(init_epoch, args.num_epoch + 1):
        ...
        for iteration, (x1, x2, x3, x4, m1, m2, m3, m4) in enumerate(data_loader):
            ...
            #grid1 = create_warp_grid(flow1, grid)
            #grid2 = create_warp_grid(flow2, grid)
            #grid3 = create_warp_grid(flow3, grid)
            ...
            # === 3. REGISTRATION LOSS CALCULATION ===
            ...
            lambda_dice = 1.0   # HYPERPARAMETER to tune
            lambda_smooth = 1e-2 # HYPERPARAMETER to tune

            errG_total = errG + (lambda_dice * loss_reg_dice) + (lambda_smooth * loss_reg_smooth)
            ...
            if is_master:
                ...
                _wandb_log({
                    ...
                    "loss/Reg/dice": loss_reg_dice.item(),
                    "loss/Reg/smooth": loss_reg_smooth.item(),
        ...
        for iteration, (x1_val, x2_val, x3_val, x4_val, m1_val, m2_val, m3_val, m4_val) in enumerate(data_loader_val):
            ...
            mask1_val = m1_val.to(device, non_blocking=True)
            mask2_val = m2_val.to(device, non_blocking=True)
            mask3_val = m3_val.to(device, non_blocking=True)
            target_mask_val = m4_val.to(device, non_blocking=True)


            # === 1. REGISTRATION STEP (Validation) ===
            reg_input_val = torch.cat([cond_data1_val, cond_data2_val, cond_data3_val, target_mask_val], dim=1)
            flow1_val, flow2_val, flow3_val = reg_net(reg_input_val)

            # Create the warp grid (re-using logic from training)
            B, _, H, W = cond_data1_val.shape
            x_coords = torch.arange(W, device=device)
            y_coords = torch.arange(H, device=device)
            mesh_x, mesh_y = torch.meshgrid(x_coords, y_coords, indexing='ij')
            base_grid_xy = torch.stack([mesh_x, mesh_y], dim=0).float()
            base_grid_xy = base_grid_xy.permute(0, 2, 1)
            base_grid_xy = base_grid_xy.unsqueeze(0).repeat(B, 1, 1, 1)

            grid1_val = create_warp_grid_with_base(flow1_val, base_grid_xy, H, W)
            grid2_val = create_warp_grid_with_base(flow2_val, base_grid_xy, H, W)
            grid3_val = create_warp_grid_with_base(flow3_val, base_grid_xy, H, W)

            # Warp images and masks for validation
            cond1_warped_val = F.grid_sample(cond_data1_val, grid1_val, mode='bilinear', padding_mode='border', align_corners=True)
            cond2_warped_val = F.grid_sample(cond_data2_val, grid2_val, mode='bilinear', padding_mode='border', align_corners=True)
            cond3_warped_val = F.grid_sample(cond_data3_val, grid3_val, mode='bilinear', padding_mode='border', align_corners=True)
            
            mask1_warped_val = F.grid_sample(mask1_val, grid1_val, mode='nearest', align_corners=True)
            mask2_warped_val = F.grid_sample(mask2_val, grid2_val, mode='nearest', align_corners=True)
            mask3_warped_val = F.grid_sample(mask3_val, grid3_val, mode='nearest', align_corners=True)

            # === 2. REGISTRATION LOSS CALCULATION (Validation) ===
            dce_loss_cond1 = dice_loss(mask1_warped_val, target_mask_val)
            dce_loss_cond2 = dice_loss(mask2_warped_val, target_mask_val)
            dce_loss_cond3 = dice_loss(mask3_warped_val, target_mask_val)
            loss_reg_dice_val = (dce_loss_cond1 + dce_loss_cond2 + dce_loss_cond3) / 3.0

            smooth_loss_cond1 = smoothness_loss(flow1_val)
            smooth_loss_cond2 = smoothness_loss(flow2_val)
            smooth_loss_cond3 = smoothness_loss(flow3_val)
            loss_reg_smooth_val = (smooth_loss_cond1 + 
                                smooth_loss_cond2 + 
                                smooth_loss_cond3) / 3.0

            # Store the validation losses
            val_dice_loss[epoch, iteration] = loss_reg_dice_val.item()
            val_smooth_loss[epoch, iteration] = loss_reg_smooth_val.item()
            val_dice_loss1[epoch, iteration] = dce_loss_cond1.item()
            val_dice_loss2[epoch, iteration] = dce_loss_cond2.item()
            val_dice_loss3[epoch, iteration] = dce_loss_cond3.item()
            val_smooth_loss1[epoch, iteration] = smooth_loss_cond1.item()
            val_smooth_loss2[epoch, iteration] = smooth_loss_cond2.item()
            val_smooth_loss3[epoch, iteration] = smooth_loss_cond3.item()

            x_t = torch.randn_like(real_data_val)

            fake_sample_val = sample_from_model(pos_coeff, gen_diffusive_1, cond1_warped_val, gen_diffusive_2,
                                                cond2_warped_val,
                                                cond3_warped_val,
        ...
        val_dice_mean = float(np.nanmean(val_dice_loss[epoch, :]))
        val_smooth_mean = float(np.nanmean(val_smooth_loss[epoch, :]))
        val_dice_mean1 = float(np.nanmean(val_dice_loss1[epoch, :]))
        val_dice_mean2 = float(np.nanmean(val_dice_loss2[epoch, :]))
        val_dice_mean3 = float(np.nanmean(val_dice_loss3[epoch, :]))
        val_smooth_mean1 = float(np.nanmean(val_smooth_loss1[epoch, :]))
        val_smooth_mean2 = float(np.nanmean(val_smooth_loss2[epoch, :]))
        val_smooth_mean3 = float(np.nanmean(val_smooth_loss3[epoch, :]))
        if is_master:
            print(val_psnr_mean)
            print(val_l1_mean)
            _wandb_log({
                ...
                "metric/val/dice_mean": val_dice_mean,
                "metric/val/smooth_mean": val_smooth_mean,
                "metric/val/dice_mean1": val_dice_mean1,
                "metric/val/dice_mean2": val_dice_mean2,
                "metric/val/dice_mean3": val_dice_mean3,
                "metric/val/smooth_mean1": val_smooth_mean1,
                "metric/val/smooth_mean2": val_smooth_mean2,
                "metric/val/smooth_mean3": val_smooth_mean3,
                "epoch": epoch,
            }, step=global_step)
