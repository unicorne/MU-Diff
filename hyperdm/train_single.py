...
# Import the simplified HyperDM wrapper
...
def train_hyperdm_mudiff_single(rank, gpu, args):
    """
    Simplified training function for a single-generator HyperDM model with a warm-up phase.
    """
    ...
    val_l1_loss = np.zeros([1, args.num_epoch+1, len(data_loader_val)])
    val_psnr_values = np.zeros([1, args.num_epoch+1, len(data_loader_val)])
    ...
    # +++ START OF CHANGES: LOAD PRE-TRAINED HYPER-NETWORK +++
    if args.pretrained_hypernet_path:
        # Load on the master process first to avoid race conditions on network storage
        if is_master:
            try:
                state_dict = torch.load(args.pretrained_hypernet_path, map_location='cpu')
                # Access the `hyper_net` module within the HyperDM model to load its state
                hyperdm_model.hyper_net.load_state_dict(state_dict)
                print(f"Successfully loaded pre-trained hyper-network from: {args.pretrained_hypernet_path}")
            except FileNotFoundError:
                print(f"WARNING: Pre-trained hyper-network not found at '{args.pretrained_hypernet_path}'. Starting from scratch.")
            except Exception as e:
                print(f"ERROR loading pre-trained hyper-network: {e}")

    hyperdm_model.to(device)
    ...
    for epoch in range(init_epoch, args.num_epoch + 1):
        ...
        # --- NEW: Check if we are in the warm-up phase ---
        ...
        for iteration, (x1, x2, x3, x4) in enumerate(data_loader):
            ...
            x_pos_sample = sample_posterior(pos_coeff, x0_predict_diff[:, [0], :], x_tp1, t)
            if is_warmup:
                errG = errG_L1 # Only use L1 loss during warm-up
            else:
            ...
            if is_master and iteration % 100 == 0:
                if is_warmup:
                    print(f'[Warm-up] Epoch {epoch}, Iter {iteration}, G-L1 Loss: {errG.item():.4f}')
                else:
        ...
        if not args.no_lr_decay:
            ...
            # Only step the discriminator scheduler after the warm-up is done
            if not is_warmup:
                scheduler_disc.step()
        
        # ... (Validation, Image Saving, and Checkpointing are unchanged) ...
        if is_master:
            if epoch % 1 == 0:
        ...
        if is_master:
            ...
            for val_iter, (x1_val, x2_val, x3_val, x4_val) in enumerate(data_loader_val):
                cond_data1_val, cond_data2_val, cond_data3_val, real_data_val = [d.to(device, non_blocking=True) for d in (x1_val, x2_val, x3_val, x4_val)]
                ...
                val_psnr_values[0, epoch, val_iter] = psnr(real_data_val_np.squeeze(), fake_sample_val_np.squeeze(), data_range=1.0)
...
def parse_arguments_hyperdm_single():
    ...
    # --- NEW: Added warmup_epochs argument ---
    parser.add_argument('--warmup_epochs', type=int, default=0, help='Number of epochs to train only the generator (with L1 loss) for stability.')
    ...
    parser.add_argument('--lambda_l1_loss', type=float, default=5.0)
    ...
    parser.add_argument('--pretrained_hypernet_path', type=str, default=None, help="Path to the pre-trained hyper-network weights file.")
