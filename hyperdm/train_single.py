...
def train_hyperdm_mudiff_single(rank, gpu, args):
    ...
    if is_master:
        ...
        run_name = f"{args.exp}-rank{rank}-{time.strftime('%Y%m%d-%H%M%S')}"
        wandb.init(
            project=os.environ.get("WANDB_PROJECT", "mudiff-hyperdm"),
            name=run_name,
            config=vars(args),
            tags=["train", "DDP", "HyperDM-Single"],
    ...
    if is_master: print('train data size:' + str(len(data_loader)))
    ...
    def _log_images(tag, cond1, cond2, cond3, pred, gt, step):
        ...
        try:
            ...
            panel = torch.cat([
                ...
                torch.clamp(to_range_0_1(cond3), 0, 1), torch.clamp(to_range_0_1(pred), 0, 1),
    ...
    for epoch in range(init_epoch, args.num_epoch + 1):
        ...
        for iteration, (x1, x2, x3, x4) in enumerate(data_loader):
            ...
            if is_master and iteration % 100 == 0:
                ...
                _wandb_log({
                    "loss/D/total": errD_real + errD_fake,
                    "loss/G/total": errG,
                    "loss/G/adv": errG_adv,
                    "loss/G/L1": errG_L1,
                }, step=global_step)
                _log_images("train/strip", cond_data1, cond_data2, cond_data3, x0_predict_diff, real_data, step=global_step)
