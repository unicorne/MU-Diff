...
# ---------------------------------------------------------------
# coding=utf-8
# Adapted to single-conditional input.
...
class NCSNpp(nn.Module):
    ...
    def __init__(self, config):
        ...
        if resblock_type == 'ddpm':
            ResnetBlock = functools.partial(ResnetBlockDDPM,
                                            ...
                                            temb_dim=nf * 4,
                                            zemb_dim=z_emb_dim)
            ConvBlock = functools.partial(ResnetBlock_Feat, act=act, in_ch=config.num_channels)
        ...
        # Downsampling heads: one for x, one for cond
        channels = config.num_channels
        if progressive_input != 'none':
            input_pyramid_ch = channels

        modules.append(ConvBlock(in_ch=channels, out_ch=nf))  # x -> nf
        modules.append(ConvBlock(in_ch=channels, out_ch=nf))  # cond -> nf

        # After concatenating x_feat + cond_feat => nf*2
        hs_c = [nf * 2]
        in_ch = nf * 2
        ...
        mapping_layers = [PixelNorm(), dense(config.nz, z_emb_dim), self.act]
    ...
    def forward(self, x, cond, time_cond, z):
        ...
        if self.embedding_type == 'fourier':
            ...
            temb = modules[m_idx](torch.log(used_sigmas)); m_idx += 1
        ...
        x_feat   = modules[m_idx](x);    m_idx += 1
        cond_feat= modules[m_idx](cond); m_idx += 1

        hs = [torch.cat((x_feat, cond_feat), dim=1)]
        ...
        return torch.tanh(h) if not self.not_use_tanh else h


@utils.register_model(name='ncsnpp_adaptive')
class NCSNpp_adaptive(nn.Module):
    """NCSN++ adaptive (single conditional + guide)"""
    ...
    def __init__(self, config):
        ...
        if resblock_type == 'ddpm':
            ResnetBlock = functools.partial(ResnetBlockDDPM,
                                            ...
                                            temb_dim=nf * 4,
                                            zemb_dim=z_emb_dim)
            ConvBlock_Adapt = functools.partial(ResnetBlock_Adapt_Feat, act=act, in_ch=config.num_channels)
            ConvBlock_Feat  = functools.partial(ResnetBlock_Feat,        act=act, in_ch=config.num_channels)
            ConvBlock_GAP   = functools.partial(ResnetBlock_Feat_GAP,    act=act, in_ch=config.num_channels)
        elif resblock_type == 'biggan':
            ResnetBlock = functools.partial(ResnetBlockBigGAN,
                                            act=act,
                                            dropout=dropout,
        ...
        # Heads: (1) pseudo_target -> weight, (2) x -> feat, (3) cond -> adapted feat
        channels = config.num_channels
        if progressive_input != 'none':
            input_pyramid_ch = channels

        modules.append(ConvBlock_GAP(in_ch=channels, out_ch=nf))  # pseudo_target -> nf
        modules.append(ConvBlock_Feat(in_ch=channels, out_ch=nf)) # x -> nf
        modules.append(ConvBlock_Adapt(in_ch=channels, out_ch=nf))# cond (adapted by pseudo)
    ...
    def forward(self, x, cond, time_cond, z, pseudo_target):
        zemb = self.z_transform(z)
        modules = self.all_modules
        m_idx = 0
        ...
        pseudo_weight = modules[m_idx](pseudo_target); m_idx += 1
        x_feat       = modules[m_idx](x);             m_idx += 1
        # adapt cond with pseudo_weight
        cond_feat    = modules[m_idx](cond, pseudo_weight); m_idx += 1
