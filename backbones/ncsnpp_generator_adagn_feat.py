...
@utils.register_model(name='ncsnpp_adaptive')
class NCSNpp_adaptive(nn.Module):
    """NCSN++ model"""
    ...
    def __init__(self, config):
        ...
        if resblock_type == 'ddpm':
            ...
            ResnetBlock = functools.partial(ResnetBlockBigGAN,
                                            ...
                                            temb_dim=nf * 4,
                                            zemb_dim=z_emb_dim)
            ConvBlock = functools.partial(ResnetBlock_Adapt_Feat,
                                          act=act, in_ch=config.num_channels
                                          )
            ConvBlock_Feat = functools.partial(ResnetBlock_Feat,
                                               act=act, in_ch=config.num_channels
                                               )
        ...
        self.feat_weight_c1 = conv3x3(nf,nf)
        self.feat_weight_c2 = conv3x3(nf, nf)
        self.feat_weight_c3 = conv3x3(nf, nf)

        self.feat_att1_c12 = conv3x3(3*nf,nf)
        self.feat_att2_c12 = conv3x3(3*nf,nf)

        self.feat_att1_c23 = conv3x3(3*nf,nf)
        self.feat_att2_c23 = conv3x3(3*nf,nf)

        self.feat_att1_c31 = conv3x3(3*nf,nf)
        self.feat_att2_c31 = conv3x3(3*nf,nf)
    ...
    def forward(self, x, cond1, cond2, cond3, time_cond, z, pseudo_target):
        # timestep/noise_level embedding; only for continuous training
        ...
        cond1_feat = modules[m_idx](cond1, pseudo_weight)
        m_idx += 1

        cond2_feat = modules[m_idx](cond2, pseudo_weight)
        m_idx += 1

        cond3_feat = modules[m_idx](cond3,pseudo_weight)
        m_idx += 1

        all_feat_att_1_c12 =  torch.sigmoid(self.feat_att1_c12(torch.cat(( cond1_feat,cond2_feat,cond3_feat ), axis=1)))
        all_feat_att_2_c12 =  torch.sigmoid(self.feat_att2_c12(torch.cat(( cond1_feat,cond2_feat,cond3_feat ), axis=1)))

        all_feat_att_1_c23 = torch.sigmoid(self.feat_att1_c23(torch.cat(( cond1_feat,cond2_feat,cond3_feat ), axis=1)))
        all_feat_att_2_c23 =  torch.sigmoid(self.feat_att2_c23(torch.cat(( cond1_feat,cond2_feat,cond3_feat ), axis=1)))

        all_feat_att_1_c31 = torch.sigmoid(self.feat_att1_c31(torch.cat(( cond1_feat,cond2_feat,cond3_feat ), axis=1)))
        all_feat_att_2_c31 = torch.sigmoid(self.feat_att2_c31(torch.cat(( cond1_feat,cond2_feat,cond3_feat ), axis=1)))
        #cond1 and cond2
        ...
        #cond2 and cond3
        cond2_att = self.feat_weight_c2(all_feat_att_1_c23*cond2_feat)
        fused_cond23 = (all_feat_att_2_c23*cond2_att) + ((1-all_feat_att_2_c23)*cond3_feat)
