...
class Loss_DGIR:
    def __init__(self, loss_cls, name):
        self.loss_cls = loss_cls
        self.name = name

    def compute_loss(self, img1, img2):
        loss_value = self.loss_cls.forward(img1, img2)
        return np.round(float(loss_value),4)
...
##################### Functions ###################
"""
From https://github.com/uncbiag/dgir/blob/main/src/diffusion_registration/core/losses.py
"""
def normalize(image):
...
class LNCC(SimilarityBase):
    ...
    def forward(self, input, target):
        return self.__call__(input, target)
...
class NewLNCC_wrapper(Loss):
    def __init__(self, model_path, name="New LNCC Loss", sigma=4, device="cpu"):
        self.sigma = sigma
        self.device = device
        ...
        model, diffusion = create_model_and_diffusion(**self.diffusion_config)
        model.load_state_dict(torch.load(model_path, map_location=self.device))
        self.new_lncc = NewLNCC(diffusion, model, sigma=self.sigma)
        super().__init__(self.new_lncc, name)
...
class BlurredSSD(SimilarityBase):
    ...
    def forward(self, input, target):
        return torch.mean((self.blur(input) - self.blur(target)) ** 2)
