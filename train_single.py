...
from dataset.dataset_dixon import CreateDatasetSynthesis_single
...
def train_mudiff(rank, gpu, args):
    ...
    from backbones.ncsnpp_generator_adagn_feat_single import NCSNpp
    from backbones.ncsnpp_generator_adagn_feat_single import NCSNpp_adaptive
    ...
    dataset = CreateDatasetSynthesis_single(phase="train", input_path=args.input_path, contrast=args.contrast1, target_contrast=args.contrast2)
    dataset_val = CreateDatasetSynthesis_single(phase="val", input_path=args.input_path, contrast=args.contrast1, target_contrast=args.contrast2)
    ...
    def _log_images(tag, cond1, pred1, pred2, gt, step):
        if not is_master:
            return
        try:
            # build a horizontal strip: cond1 | pred1 | pred2 | gt
            ...
            wandb.log({tag: wandb.Image(grid.cpu(), caption="cond1|pred_g1|pred_g2|gt")}, step=step)
        except Exception:
            pass
    ...
    for epoch in range(init_epoch, args.num_epoch + 1):
        ...
        for iteration, (x1, x4) in enumerate(data_loader):
            ...
            with torch.no_grad():
                x2_0_predict_diff_g1 = gen_diffusive_1(x2_tp1.detach(), cond_data1, t2, latent_z2)

                x2_0_predict_diff_g2 = gen_diffusive_2(x2_tp1.detach(), cond_data1, t2, latent_z2,
        ...
        if rank == 0:
            ...
            fake_sample = sample_from_model_single(pos_coeff, gen_diffusive_1, cond_data1, gen_diffusive_2,
        ...
        for iteration, (x1_val, x4_val) in enumerate(data_loader_val):
            ...
            fake_sample_val = sample_from_model_single(pos_coeff, gen_diffusive_1, cond_data1_val, gen_diffusive_2,
        ...
        if is_master:
            print("Val PSNR: ", val_psnr_mean)
            print("Val L1: ", val_l1_mean)
            _wandb_log({
